...
import type { HttpRequest, Method } from 'src/interfaces/httpRequest'
...
export class TappyToon extends BaseSite {
  ...
  protected override getChapter(data: TappyToonData): string {
    return data.getChapterData()?.title ?? 'Unknown'
  }

  protected override getChapterUrl(data: TappyToonData): string {
  ...
  protected override getChapterNum(data: TappyToonData): number {
    return data.getChapterData()?.order ?? 0
  }

  protected override getChapterDate(data: TappyToonData): string {
  ...
  protected override getImage(data: TappyToonData): string {
    return data.comicData.posterThumbnailUrl
  }

  protected override getTitle(data: TappyToonData): string {
  ...
  private async getChapterDataList(
    nextData: NextData,
    headers: Record<string, string>,
  ): Promise<ChapterData[] | Error> {
  ...
  protected async searchImpl(query: string): Promise<Error | Manga[]> {
    ...
    searchData.forEach((entry) => {
      ...
      promises.push(
        new Promise((resolve) => {
          this.getChapterDataListById(entry.id, headers)
            .then((chapterDataList) => {
